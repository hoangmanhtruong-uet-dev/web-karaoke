...
describe("booking validation", () => {
  it("rejects a missing branch", () => {
    expect(
      bookingInputSchema.safeParse({ ...validInput, branchId: "" }).success
    ).toBe(false)
  })

  it.each([0, -1])("rejects guestCount %s", (guestCount) => {
    expect(
      bookingInputSchema.safeParse({ ...validInput, guestCount }).success
    ).toBe(false)
  })

  it.each([0, -1, 13, 1.5])("rejects durationHours %s", (durationHours) => {
    expect(
      bookingInputSchema.safeParse({ ...validInput, durationHours }).success
    ).toBe(false)
  })

  it("canonicalizes Vietnam country-code phones before duplicate checks", () => {
    const parsed = bookingInputSchema.parse({
      ...validInput,
      customerPhone: "+84 901 234 567",
    })

    expect(parsed.customerPhone).toBe("0901234567")
  })
  ...
  it("converts Vietnam local time to UTC and rejects past bookings", () => {
    ...
    expect(
      validateBookingWindow(window, new Date("2030-07-21T13:00:00.000Z"))
    ).toContain("tương lai")
  })
...
describe("booking overlap", () => {
  ...
  it.each([
    ...
    "handles %s-%s vs %s-%s: %s (%s)",
    (aStart, aEnd, bStart, bEnd, expected) => {
      expect(
        intervalsOverlap(
          date(aStart as number),
          date(aEnd as number),
          date(bStart as number),
          date(bEnd as number)
        )
      ).toBe(expected)
    }
  ...
  function record(
    overrides: Partial<BookingConflictRecord> = {}
  ...
  it("allows the same time in another room", () => {
    expect(bookingBlocksSlot(record(), record({ roomId: "room-2" }))).toBe(
      false
    )
  })

  it("allows the same time in another branch", () => {
    expect(bookingBlocksSlot(record(), record({ branchId: "branch-2" }))).toBe(
      false
    )
  })

  it("does not let a cancelled booking block the slot", () => {
    expect(bookingBlocksSlot(record({ status: "cancelled" }), record())).toBe(
      false
    )
  })

  it("does not let an expired booking block the slot", () => {
    expect(bookingBlocksSlot(record({ status: "expired" }), record())).toBe(
      false
    )
  })
})
...
describe("idempotency request hash", () => {
  ...
  it("changes when the payload changes", () => {
    expect(hashBookingRequest(validInput)).not.toBe(
      hashBookingRequest({
        ...validInput,
        guestCount: validInput.guestCount + 1,
      })
    )
  })
})
