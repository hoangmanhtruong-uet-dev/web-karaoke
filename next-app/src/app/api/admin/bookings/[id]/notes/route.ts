...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){const origin=requireSameOrigin(request);if(origin)return origin;const auth=await authorizeAdminApi();if(!hasPrincipal(auth))return auth.response;const parsed=schema.safeParse(await request.json().catch(()=>null));if(!parsed.success)return apiError(422,"VALIDATION_ERROR","Ghi chú không hợp lệ.",parsed.error.flatten().fieldErrors);const{id}=await params;return apiSuccess(await addAdminNote({bookingId:id,content:parsed.data.content,actor:auth.principal}),201)}
