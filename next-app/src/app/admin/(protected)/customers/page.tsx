...
export default async function AdminCustomersPage() {
  const admin = await requirePermissionPage("customer.read")
  const branchId = resolveAdminBranchId(admin)
  const customers = await prisma.customer.findMany({ where: branchId ? { bookings: { some: { branchId } } } : {}, orderBy: { updatedAt: "desc" }, take: 100, include: { _count: { select: { bookings: branchId ? { where: { branchId } } : true } } } })
